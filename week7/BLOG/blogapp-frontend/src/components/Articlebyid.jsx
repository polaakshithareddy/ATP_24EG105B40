...
//otherwise,make api req to read that article by id
  ...
  const toggleArticleStatus = async () => {
    ...
    try {
      const res = await axios.patch(
        `${import.meta.env.VITE_API_BASE_URL}/author-api/article`,
        {articleId:article._id,isArticleActive:newStatus},
        { withCredentials: true },//token is passed with req
      );
  ...
  const addComment = async (commentObj) => {
    ...
    let res = await axios.put(`${import.meta.env.VITE_API_BASE_URL}/user-api/article`, commentObj, { withCredentials: true });
    if (res.status === 200) {
